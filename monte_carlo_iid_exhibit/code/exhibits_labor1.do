...
local popu = 2.488905
use "../input/sum_iid_labor_`1'_`popu'_`3'.dta", clear
gen pop = `popu'
...
shell echo -n `slope_csp_median' > "../output/csp_slope_median_iid_labor_`1'_`popu'_`3'.tex"
shell echo -n `intercept_csp_median' > "../output/csp_intercept_median_iid_labor_`1'_`popu'_`3'.tex"
...
graph export "../output/slopes_intercepts_densities_iid_labor.eps", replace fontface("Times")
// graph export is not compatible with decimal numbers in filenames
shell mv "../output/slopes_intercepts_densities_iid_labor.eps" ///
	"../output/slopes_intercepts_densities_iid_labor_`1'_`popu'_`3'.eps"


quietly summarize mse_cbm, detail
local cbm_mse_mean = string(`r(mean)', "%4.2f") 
shell echo -n `cbm_mse_mean' > "../output/cbm_mse_mean_iid_labor_`1'_`popu'_`3'.tex"

quietly summarize mse_csp, detail
local csp_mse_mean = string(`r(mean)', "%4.2f")
shell echo -n `csp_mse_mean' > "../output/csp_mse_mean_iid_labor_`1'_`popu'_`3'.tex"
...
shell echo -n `mse_ratio_median' > "../output/mse_ratio_median_iid_labor_`1'_`popu'_`3'.tex"
local mse_ratio_sd = string(`r(sd)', "%4.2f")
shell echo -n `mse_ratio_sd' > "../output/mse_ratio_sd_iid_labor_`1'_`popu'_`3'.tex"

// Plot histogram of MSE ratio
sum ratio_MSE, detail 
local ratio_MSE_start = floor(`r(min)' * 10) / 10 
twoway (hist ratio_MSE, lcolor(black) fcolor(none) start(`ratio_MSE_start') fraction), ///
	graphregion(color(white)) xtitle("Covariates-based MSE / Calibrated-shares MSE") ///
	xlabel(#6) legend(off)
graph export "../output/MSE_ratio_histogram_iid_labor.eps", replace fontface("Times")
shell mv "../output/MSE_ratio_histogram_iid_labor.eps" ///
	"../output/MSE_ratio_histogram_iid_labor_`1'_`popu'_`3'.eps" 
