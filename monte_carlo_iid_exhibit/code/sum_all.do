...
foreach pop of numlist 2.488905 5 12.5 25 50 125 250 2560 {
	foreach Lambda of numlist 0 0.1 0.25 0.5 1{
		...
		if `Lambda'==0.25 local Lambda="0.25" // otherwise shown as .1 and stata cannot find the file
		if `Lambda'==0.5 local Lambda="0.5" // otherwise shown as .1 and stata cannot find the file
...
frmttable using "../output/monte_carlo_result_mean.tex", statmat(mat_output_mean) ///
...
shell sed -i.bak 's/\\end{tabular}\\\\/\\end{tabular}/g' "../output/monte_carlo_result_mean.tex"
rm ../output/monte_carlo_result_mean.tex.bak
