...
# shares
df = DataFrame(load("../temp/nyc_2010_levels_tracttotract_approx_nnmf_"*string(rank)*".dta"))
df = sort(df, [:j, :i])
...
tuple = (model_class = "nnmf_" * string(rank),
    α = alpha, 
    ε = epsilon, 
    σ = sigma, 
    η = eta, 
...
save("../output/model_nnmf_"*string(rank)*".jld2", "model_parameters", tuple)
